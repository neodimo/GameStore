...
Version 0.11 is an Electron + React/TypeScript desktop application with matching Windows and Linux functionality. Its desktop shell follows the approved mockup direction: fixed navigation rail, global search, platform/filter bars, dense art-led shelves, and anchored inline details. It contains a selected 268-game PS1 library: official English-language releases plus the existing explicitly documented English fan translations. It deliberately excludes duplicate regional printings and a raw full-ROM-set dump. The app includes local favorites/export, filters, Weird/Horror/Arcade/RPG shelves, translation records, media/video states, managed downloads, a persistent MiSTer cart, and stateful outbound sources. GitHub Actions builds a Windows NSIS installer plus Linux AppImage and Debian packages.
...
Expanded details place a larger cover, description, and facts in a left rail, while the right media column carries acquisition, a looping preview, and large primary-release gameplay screenshots. Libretro `Named_Snaps` and `Named_Titles` resolve through the same strict title/release matcher as artwork; preview frames augment that gallery. Users sign into EmuMovies directly in Settings with the separately generated FTP/file-server credentials (often different from their emumovies.com website email/password); the username and encrypted password remain install-local, the FTP index is cached, and exact filename matches supply compact video snaps. An FTP rejection is reported as an authentication failure, never inferred to be a membership downgrade. When EmuMovies is unavailable, a strict Internet Archive match streams a short byte range from a longplay rather than downloading it. Settings reports cache bytes/path and provides a clear action that does not erase user curation.
