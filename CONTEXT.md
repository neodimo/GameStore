...
- Never bundle video. Resolve it from attributed providers, show the derivative size, and cache it only after explicit approval; playback uses the local file. Bundled media is limited to small app-owned assets such as the icon, logo, loading UI, and missing-media placeholders.
...
Expanded details keep screenshots and video visible together. Libretro `Named_Snaps` and `Named_Titles` resolve through the same strict title/release matcher as artwork and cache after the user opens a game. Internet Archive's PSX longplay corpus supplies optional MP4 derivatives; a strict matcher rejects near-misses, the UI shows the selected derivative's size, and playback begins only after the complete file is in GameStore's local cache. Settings reports cache bytes/path and provides a clear action that does not erase user curation.

Media discovery now begins 600 ms after startup as a non-blocking three-worker audit. It shares one set of cached provider indexes, fills missing screenshots, resolves longplay availability/size/cached state, and publishes per-title records for instant detail rendering. Opening a title reuses or prioritizes the same in-flight promise, preventing duplicate provider work. The header reports progress; clearing the disposable media cache resets and reruns the audit. Full video binaries remain explicit size-disclosed downloads because the matched longplays can total many gigabytes.
